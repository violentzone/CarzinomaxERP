"""Initial schema - every table as of the payment-categories change.

Frozen baseline generated from Base.metadata. Pre-Alembic databases (created
by the old create_all flow) are stamped at this revision on startup instead of
running it; 0002 then reconciles the payments table shape idempotently.

Note: departments <-> employees reference each other (manager_id / and
employees.department_id), so departments is created without its manager FK,
which is added by ALTER once employees exists — the same way create_all
resolves the cycle.
...
def upgrade() -> None:
    ...
    # departments.manager_id FK added after employees exists (cycle breaker)
...
def downgrade() -> None:
    ...
    # cycle breaker: detach departments.manager_id before employees goes away
